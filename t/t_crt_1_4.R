...
alert_data_al <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]!='RealizaAuditoria' AND Rol!='Coordinador' AND codigo_amparo!=11 AND codigo_amparo!=12 AND [Nombre Bandeja]='AnalisisLiquidacion' AND [usuario que remite a bandeja]='conciliacionSE' AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))

alert_data_arc <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]!='RealizaAuditoria' AND Rol!='Coordinador' AND codigo_amparo!=11 AND codigo_amparo!=12 AND [Nombre Bandeja]='ActualizacionRespuestasCartas' AND [usuario que remite a bandeja]='conciliacionSE' AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
...
if(nrow(alert_data_arc) > 0 ){
	...
	dtab = dbGetQuery(con,"SET search_path TO alert_sch;")
	dbWriteTable(con, "alert_soat_se_siniestro_bandeja", alert_data_arc, row.names=FALSE, append=TRUE)
	print("Pg: ARC OK")
}
