...
q_pg = paste0("SET search_path TO alert_sch; SELECT 1 AS pag, ASSS.a_siniestro, ASSS.nro_siniestro, ASSS.nro_cuenta FROM alert_sch.alert_soat_se_siniestro ASSS WHERE (estado=2 OR estado=3) AND nro_siniestro < 10000000 AND NOT EXISTS (SELECT * FROM alert_sch.alert_soat_se_siniestro_estado ASSSC WHERE ASSSC.a_siniestro=ASSS.a_siniestro AND ASSSC.nro_siniestro=ASSS.nro_siniestro AND ASSSC.nro_cuenta=ASSS.nro_cuenta) LIMIT 500;")
