...
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='RealizaAuditoria' AND Rol LIKE 'Auditor%' AND Rol LIKE '%Administrativo' AND Rol!='Coordinador' AND NOT (codigo_amparo=11 OR codigo_amparo=12);"))
