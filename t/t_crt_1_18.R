...
#-- Caso: (Bandeja: RealizaAuditoriaExterna, Rol: AuditorExterno)
t_ini <- Sys.time()
t_ini
...
#alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,-1, getdate())) AND [Nombre Bandeja]='RealizaAuditoriaExterna' AND Rol LIKE 'Auditor Externo' AND NOT (codigo_amparo=11 OR codigo_amparo=12);"))
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='RealizaAuditoriaExterna' AND Rol LIKE 'Auditor Externo' AND NOT (codigo_amparo=11 OR codigo_amparo=12) AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
...
print("SqlSvr: RealizaAuditoriaExterna-AuditorExterno OK")
...
if(nrow(alert_data) > 0){
	...
	alert_data$id_bandeja <- 22
	alert_data$estado <- 21
	...
	print("Pg: RealizaAuditoriaExterna-AuditorExterno OK")
}
