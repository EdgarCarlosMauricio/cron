...
alert_data <- dbGetQuery(uat_conn,paste("use vida_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='VIDA-APE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='AutorizacionCarta' AND Rol LIKE 'Auditor%' AND Rol LIKE '%Administrativo' AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
...
if(nrow(alert_data) > 0){
	...
	alert_data$id_bandeja <- 27
	alert_data$estado <- 27
