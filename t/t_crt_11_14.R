...
alert_data <- dbGetQuery(uat_conn,paste("use vida_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='VIDA-APE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='RealizaAuditoria' AND Rol='Auditor Administrativo' AND Rol!='Coordinador' AND [usuario que remite a bandeja]!='conciliacionAPE' AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
