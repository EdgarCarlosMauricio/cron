...
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='AnalisisLiquidacion' AND Rol='Analista' AND [usuario que remite a bandeja]!='conciliacionSE' AND codigo_estado_cuenta='EGRC' AND NOT ((codigo_amparo=2 OR codigo_amparo=3 OR codigo_amparo=6) AND [usuario que remite a bandeja]!='conciliacionSE') AND NOT (codigo_amparo=11 OR codigo_amparo=12);"))

alert_data_o <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='AnalisisLiquidacion' AND Rol LIKE 'Auditor M%' AND Rol LIKE '%dico' AND [usuario que remite a bandeja]!='conciliacionSE' AND NOT ((codigo_amparo=2 OR codigo_amparo=3 OR codigo_amparo=6) AND [usuario que remite a bandeja]!='conciliacionSE') AND NOT (codigo_amparo=11 OR codigo_amparo=12);"))
...
if(nrow(alert_data_o) > 0){
    id_bandeja_o <- 9
    query_update_ini_o = paste0("SET search_path TO alert_sch; UPDATE alert_sch.alert_soat_se_siniestro_bandeja SET estado=0, fecha_registro=CURRENT_TIMESTAMP WHERE (nro_siniestro, a_siniestro, nro_cuenta, id_siniestro_bandeja) IN (SELECT A.nro_siniestro, A.a_siniestro, A.nro_cuenta, A.id_siniestro_bandeja FROM alert_sch.alert_soat_se_siniestro_bandeja A WHERE A.id_siniestro_bandeja=(SELECT max(id_siniestro_bandeja) FROM alert_sch.alert_soat_se_siniestro_bandeja B WHERE B.nro_siniestro=A.nro_siniestro AND B.a_siniestro=A.a_siniestro AND B.nro_cuenta=A.nro_cuenta GROUP BY B.nro_siniestro, B.a_siniestro, B.nro_cuenta) AND A.estado=11 AND A.id_bandeja=",id_bandeja_o," ")
    filter_upd_o = "";
