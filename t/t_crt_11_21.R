...
#-- Caso: (Bandeja: PreRevision, TipoEntidad=PJ y PN)
t_ini <- Sys.time()
t_ini
...
#Persona Juridica
alert_data_pj <- dbGetQuery(uat_conn,paste("use vida_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='VIDA-APE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja] LIKE 'PreRevision' AND Rol='AudExterna' AND tipo_entidad!='PN' AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
...
if(nrow(alert_data_pj) > 0){
	...
	alert_data_pj$id_bandeja <- 28
	alert_data_pj$estado <- 28
...
if(nrow(alert_data_pn) > 0){
	...
	alert_data_pn$id_bandeja <- 29
	alert_data_pn$estado <- 29
