...
#-- Caso: (Bandeja: AutorizacionCarta, Rol: AuditorMedico)
t_ini <- Sys.time()
t_ini
...
#alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,-1, getdate())) AND [Nombre Bandeja]='AutorizacionCarta' AND Rol LIKE 'Auditor M%' AND Rol LIKE '%dico' AND NOT (codigo_amparo=11 OR codigo_amparo=12);"))
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='AutorizacionCarta' AND Rol LIKE 'Auditor M%' AND Rol LIKE '%dico' AND NOT (codigo_amparo=11 OR codigo_amparo=12) AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
...
print("SqlSvr: AutorizacionCarta-AuditorMedico OK")
...
if(nrow(alert_data) > 0){
	...
	alert_data$id_bandeja <- 11
	alert_data$estado <- 15
	...
	print("Pg: AutorizacionCarta-AuditorMedico OK")
}
