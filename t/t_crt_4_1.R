...
q_pg = paste0("SET search_path TO alert_sch; SELECT ASSS.nro_siniestro||'/'||ASSS.a_siniestro||'*'||ASSS.nro_cuenta AS nro_siniestro FROM alert_sch.alert_soat_se_siniestro ASSS;")
...
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT DISTINCT NroSiniestro AS nro_siniestro FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,+1, getdate())) AND NroSiniestro != '';"))
...
if(length(diffSAC)>0){
    ...
    for (i_s in diff_sac_tmp) {
        i = i + 1
        query_insert = paste0(query_insert,"INSERT INTO alert_sch.alert_soat_se_siniestro VALUES(",i_s[1],",",i_s[2],",",i_s[3],",712,CURRENT_TIMESTAMP,1);")
