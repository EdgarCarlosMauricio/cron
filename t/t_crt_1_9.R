...
#-- Caso: (Bandeja: RealizaAuditoria, Rol: AuditorMedico)
t_ini <- Sys.time()
t_ini
...
#alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,-1, getdate())) AND [Nombre Bandeja]='RealizaAuditoria' AND Rol LIKE 'Auditor M%' AND Rol LIKE '%dico' AND Rol!='Coordinador' AND NOT ((codigo_amparo=2 OR codigo_amparo=3 OR codigo_amparo=6) AND [usuario que remite a bandeja]!='conciliacionSE') AND NOT (codigo_amparo=11 OR codigo_amparo=12);"))
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='RealizaAuditoria' AND Rol LIKE 'Auditor M%' AND Rol LIKE '%dico' AND Rol!='Coordinador' AND NOT ((codigo_amparo=2 OR codigo_amparo=3 OR codigo_amparo=6 OR tipo_entidad='PN') AND [usuario que remite a bandeja]!='conciliacionSE') AND NOT (codigo_amparo=11 OR codigo_amparo=12) AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
...
print("SqlSvr: RealizaAuditoria-Auditor Medico OK")
...
if(nrow(alert_data) > 0){
	...
	alert_data$id_bandeja <- 3
	alert_data$estado <- 12
	...
	print("Pg: RealizaAuditoria-Auditor Medico OK")
}
