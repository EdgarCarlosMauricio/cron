...
q_pg = paste0("SET search_path TO alert_sch; SELECT ASSS.nro_siniestro||'/'||ASSS.a_siniestro||'*'||ASSS.nro_cuenta AS nro_siniestro FROM alert_sch.alert_soat_se_siniestro ASSS WHERE (ASSS.nro_siniestro, ASSS.a_siniestro, ASSS.nro_cuenta) IN (SELECT A.nro_siniestro, A.a_siniestro, A.nro_cuenta FROM alert_sch.alert_soat_se_siniestro_bandeja A WHERE A.id_siniestro_bandeja=(SELECT max(id_siniestro_bandeja) FROM alert_sch.alert_soat_se_siniestro_bandeja B WHERE B.nro_siniestro=A.nro_siniestro AND B.a_siniestro=A.a_siniestro AND B.nro_cuenta=A.nro_cuenta GROUP BY B.nro_siniestro, B.a_siniestro, B.nro_cuenta) AND A.estado>0 AND (A.id_bandeja=7 OR A.id_bandeja=8 OR A.id_bandeja=3));")
...
alert_data <- dbGetQuery(uat_conn,paste("USE soat_gruposis; SELECT BG.NroSiniestro AS nro_siniestro FROM cuentas CU WITH (NOLOCK) INNER JOIN GrupoSIS..Bandejas_General BG WITH (NOLOCK) ON CONCAT(CU.nro_siniestro,'/',CU.a_siniestro,'*',CU.nro_cuenta)=BG.NroSiniestro WHERE CU.metodo_pago=7 AND BG.equipo='SOAT-SE' AND BG.fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate()));"))
