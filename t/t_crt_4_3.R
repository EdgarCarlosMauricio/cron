...
q_pg = paste0("SET search_path TO alert_sch; SELECT ASSSC.nro_siniestro||'/'||ASSSC.a_siniestro||'*'||ASSSC.nro_cuenta AS nro_siniestro, ASSSC.fecha_ultimo_documento FROM alert_sch.alert_soat_se_siniestro_estado ASSSC WHERE ASSSC.id_estado=(SELECT max(id_estado) FROM alert_sch.alert_soat_se_siniestro_estado WHERE nro_siniestro=ASSSC.nro_siniestro AND a_siniestro=ASSSC.a_siniestro AND nro_cuenta=ASSSC.nro_cuenta);")
...
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT DISTINCT NroSiniestro AS nro_siniestro, ISNULL(Fecha_Ultimo_Documento, '1970/01/01') AS fecha_ultimo_documento FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,+1, getdate())) AND NroSiniestro != '';"))
...
if(nrow(data_diff) > 0){
    ...
    for(i in 1:nrow(data_diff)){
        ...
        query_insert = paste0(query_insert,"UPDATE alert_sch.alert_soat_se_siniestro_estado SET fecha_ultimo_documento='",data_diff$f_u_d_year[i],"-",data_diff$f_u_d_mes[i],"-",data_diff$f_u_d_dia[i],"', fecha_registro=CURRENT_TIMESTAMP WHERE (nro_siniestro, a_siniestro, nro_cuenta, id_estado) = (SELECT A.nro_siniestro, A.a_siniestro, A.nro_cuenta, A.id_estado FROM alert_sch.alert_soat_se_siniestro_estado A WHERE A.nro_siniestro=",data_diff$nro_siniestro[i]," AND A.a_siniestro=",data_diff$a_siniestro[i]," AND A.nro_cuenta=",data_diff$nro_cuenta[i]," AND id_estado=(SELECT max(id_estado) FROM alert_sch.alert_soat_se_siniestro_estado WHERE nro_siniestro=A.nro_siniestro AND a_siniestro=A.a_siniestro AND nro_cuenta=A.nro_cuenta) AND A.fecha_ultimo_documento<'",data_diff$f_u_d_year[i],"-",data_diff$f_u_d_mes[i],"-",data_diff$f_u_d_dia[i],"');")
