...
q_pg = paste0("SET search_path TO alert_sch; SELECT ASSS.nro_siniestro||'/'||ASSS.a_siniestro||'*'||ASSS.nro_cuenta AS nro_siniestro FROM alert_sch.alert_ape_se_siniestro ASSS WHERE NOT EXISTS (SELECT * FROM alert_sch.alert_ape_se_siniestro_bandeja ASSSB WHERE ASSS.nro_siniestro=ASSSB.nro_siniestro AND ASSS.a_siniestro=ASSSB.a_siniestro AND ASSS.nro_cuenta=ASSSB.nro_cuenta AND ASSSB.id_bandeja=23 AND ASSSB.estado > 0);")
...
alert_data <- dbGetQuery(uat_conn,paste("USE vida_gruposis; SELECT BG.NroSiniestro AS nro_siniestro FROM GrupoSIS..Bandejas_General BG WITH (NOLOCK) WHERE BG.equipo='VIDA-APE' AND BG.fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate()));"))
