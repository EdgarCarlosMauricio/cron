...
#-- Caso: (Bandeja: SupervisorCrearReclamante, Rol: SupervisorCrearReclamante)
t_ini <- Sys.time()
t_ini
...
#alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,-1, getdate())) AND [Nombre Bandeja]='SupervisorCrearReclamante' AND Rol LIKE 'SupervisorCrearReclamante' AND NOT (codigo_amparo=11 OR codigo_amparo=12);"))
alert_data <- dbGetQuery(uat_conn,paste("use soat_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='SupervisorCrearReclamante' AND Rol LIKE 'SupervisorCrearReclamante' AND NOT (codigo_amparo=11 OR codigo_amparo=12) AND fecha_bandeja >= convert(date,dateadd(day,-7, getdate()));"))
...
print("SqlSvr: SupervisorCrearReclamante-SupervisorCrearReclamante OK")
...
if(nrow(alert_data) > 0){
	...
	alert_data$id_bandeja <- 25
	alert_data$estado <- 25
	...
	print("Pg: SupervisorCrearReclamante-SupervisorCrearReclamante OK")
}
