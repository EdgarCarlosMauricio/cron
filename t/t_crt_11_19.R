...
#-- Caso: Objeciones Operativas (OB74)(PJ+PN) (Bandeja: AutorizacionCarta, Rol: Coordinador)
t_ini <- Sys.time()
t_ini
...
#Persona Juridica
alert_data_pj <- dbGetQuery(uat_conn,paste("use vida_gruposis; SELECT NroSiniestro AS nro_siniestro, fecha_bandeja FROM GrupoSIS..Bandejas_General WHERE equipo='VIDA-APE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND [Nombre Bandeja]='AutorizacionCarta' AND Rol='Coordinador' AND codigo_objecion = 'OB74' AND tipo_entidad!='PN';"))
...
if(nrow(alert_data_pj) > 0){
	...
	alert_data_pj$id_bandeja <- 24
	alert_data_pj$estado <- 24
...
if(nrow(alert_data_pn) > 0){
	...
	alert_data_pn$id_bandeja <- 25
	alert_data_pn$estado <- 25
