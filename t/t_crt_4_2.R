...
#-- Check 
#Old: ping sis-db-01 172.30.25.11 (20210215) 
#New: ping sis-db-03 172.30.0.39
t_ini <- Sys.time()
t_ini
...
sac_filter = "202075/2019*1"
#sac_filter = "3668/2019*2" #Cuenta fantasma: 9984/2007*1
print(paste0("SAC: ",sac_filter))
query_cuentas_srv = paste0("use soat_gruposis; SELECT nro_siniestro, a_siniestro, nro_cuenta, codigo_objecion, codigo_usuario_radica, codigo_sucursal_Recepcion, codigo_sucursal_Radica, codigo_sucursal_Procesa, codigo_sucursal_Paga, codigo_sucursal_Egreso, codigo_estado_cuenta, identificacion_reclamante, ciudad_reclamante, fecha_aviso, fecha_limite_pago, fecha_objecion, fecha_prescripcion, fecha_factura,fecha_radicacion, fecha_uactualiza, fecha_respuesta, codigo_usuario_actualiza, nro_factura, nro_cuenta_cobro, valor_cobrado, codigo_recepcion, Autorizacion_Coordinador, Terminada, EquipoRA, fecha_muerte, fecha_ingreso, fecha_egreso, metodo_pago, nro_siniestro_sise, revisada_SE, usuario_SE, fecha_revision_SE, nro_autorizacion, nro_amparo_cliente, fecha_aviso_cliente, id_direccionamiento, codigo_fami AllowedUsers, PersistentActivity, AllowedRoles, InstanceId, EventName, isOdontologia, IdCausalNovedad, codigo_clase_vehiculo, Nro_Nota_Credito, Valor_Nota_Credito, Fecha_Nota_Credito FROM soat_gruposis..cuentas WHERE CAST(nro_siniestro AS varchar)+'/'+CAST(a_siniestro AS varchar)+'*'+CAST(nro_cuenta AS varchar)='",sac_filter,"';")
data_cuentas_srv <- dbGetQuery(uat_conn,query_cuentas_srv)
print("Cuentas")
print(data_cuentas_srv)

query_sac_srv = paste0("use soat_gruposis; SELECT [Nombre Bandeja] AS 'nombre_bandeja', Rol AS rol, paquete, estado_paquete, NroSiniestro AS s_a_c, codigo_estado_cuenta AS estado_cuenta, ISNULL (codigo_objecion,'') AS objecion, CONVERT (VARCHAR (10),fecha_bandeja,103) AS fecha_bandeja, Fecha_Ultimo_Documento, codigo_amparo, [usuario que remite a bandeja] AS 'usuario_bandeja', tipo_entidad FROM GrupoSIS..Bandejas_General WHERE equipo = 'SOAT-SE' AND fecha_bandeja BETWEEN '2000-01-01' AND convert(date,dateadd(day,1, getdate())) AND NroSiniestro='",sac_filter,"' ORDER BY [Nombre Bandeja], Rol;")
...
query_bandeja_pg = paste0("SET search_path TO alert_sch; SELECT * FROM alert_sch.alert_soat_se_siniestro_bandeja A WHERE A.nro_siniestro||'/'||A.a_siniestro||'*'||A.nro_cuenta = '",sac_filter,"' ORDER BY A.nro_siniestro ASC, A.a_siniestro ASC, A.nro_cuenta ASC, A.id_siniestro_bandeja ASC;")
data_bandeja_pg = dbGetQuery(con,query_bandeja_pg)
print("Bandejas")
print(data_bandeja_pg)

query_estado_pg = paste0("SET search_path TO alert_sch; SELECT B.clave, A.* FROM alert_sch.alert_soat_se_siniestro_estado A INNER JOIN alert_sch.estado_cuenta B ON A.id_estado_cuenta=B.id_estado_cuenta WHERE A.nro_siniestro||'/'||A.a_siniestro||'*'||A.nro_cuenta = '",sac_filter,"';")
